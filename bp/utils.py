...
class BpDist(Function):
    ...
    def forward(ctx, xy, idx, Valid, num, H, W):
        ...
        for b in range(B):
            v = Valid[b:b + 1].view(1, 1, N)
            v_cnt = int(v.sum().item())

            # ✅ 关键：该尺度没有任何有效点 -> 跳过 CUDA kernel
            if v_cnt == 0:
                # IPCnum/args 默认是 0，保持即可
                continue

            Pc = torch.masked_select(xy, v).reshape(1, 2, -1).contiguous()
            BpOps.Dist(Pc, IPCnum[b:b + 1], args[b:b + 1], H, W)

            idx_valid = torch.masked_select(idx, v)
            # idx_valid 长度就是 v_cnt，index_select 安全
            args[b:b + 1] = torch.index_select(
                idx_valid, 0, args[b:b + 1].reshape(-1)
            ).reshape(1, num, N)
        return IPCnum, args
...
class Dist(nn.Module):
    ...
    def forward(self, S, xx, yy):
        ...
        S = S.reshape(B, 1, N)
        Valid = (S > 1e-3)
        xy = torch.stack((xx, yy), axis=0).reshape(1, 2, -1).float() #(2,H,W)->(1,2,N)
...
class Prop(nn.Module):
    ...
    def forward(self, If, Pf, Ofnum, args):
        ...
        X = torch.cat([Ifnum, IPfnum, Pfnum, Ofnum], dim=1) #特征融合->(B,TC,4/num,N)
        XF = self.convXF(X)
...
class PMP(nn.Module):
    ...
    def pinv(self, S, K, xx, yy):
        ...
        S = S.view(S.shape[0], 1, -1) #(B,1,N)
        xx = xx.reshape(1, 1, -1) #(1,1,H*W)->内存利用
        yy = yy.reshape(1, 1, -1) #S-yy:将二维转化为一维向量
        Px = S * (xx - cx) / fx
        Py = S * (yy - cy) / fy
        Pz = S
    ...
    def forward(self, fout, dout, XI, S, K):
        ...
        xx, yy = torch.meshgrid(torch.arange(width, device=Sp.device), torch.arange(height, device=Sp.device),
                                indexing='xy') #(H*W),不论H*W上的数据如何，相当于分别统计每一个坐标上的行与列
        ###############################################################
        # Pre
